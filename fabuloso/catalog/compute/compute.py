...
@task
def configure_libvirt(hostname, shared_storage=False,instances_path='/var/lib/nova/instances'):
    utils.uncomment_property(LIBVIRT_QEMU_CONF, 'cgroup_device_acl')
    utils.modify_property(LIBVIRT_QEMU_CONF, 'cgroup_device_acl',
...
@task
def configure_files(management_ip, rabbit_password='guest', mysql_username='nova',
                    mysql_password='stackops', mysql_host='127.0.0.1', mysql_port='3306', mysql_schema='nova',
                    service_user='nova', service_tenant_name='service', service_pass='stackops', auth_host='127.0.0.1',
                    auth_port='35357', auth_protocol='http', quantum_url='http://127.0.0.1:9696',admin_auth_url='http://127.0.0.1:35357/v2.0', libvirt_type='kvm', rabbit_host='127.0.0.1',
                    vncproxy_host='127.0.0.1', vncproxy_port='6080', glance_host='127.0.0.1', glance_port='9292'):
    ...
    utils.set_option(NOVA_COMPUTE_CONF, 'resume_guests_state_on_host_boot', 'false')
    ...
    utils.set_option(NOVA_COMPUTE_CONF, 'vncserver_proxyclient_address', management_ip)
...
def configure_ovs_plugin_gre(mysql_username='quantum',
    ...
    utils.set_option(OVS_PLUGIN_CONF,'tunnel_id_ranges','1:1000',section='OVS')
...
def get_memory_available():
    return 1024 * int(sudo("cat /proc/meminfo | grep 'MemTotal' | sed 's/[^0-9\.]//g'"))

@task
def configure_hugepages(is_enabled=False,percentage='60'):
    ...
    sudo("sed -i '/hugepages/d' /usr/share/pyshared/nova/virt/libvirt.xml.template")
    with settings(warn_only=True):
        sudo('rm etc/sysctl.d/60-hugepages.conf')
        sudo('umount /dev/hugepages')
    if is_enabled:
        pages = int(0.01 * int(percentage) * int(get_memory_available() / PAGE_SIZE)) + BONUS_PAGES
        sudo("echo '  owner /dev/hugepages/libvirt/qemu/* rw,' >> /etc/apparmor.d/abstractions/libvirt-qemu")
        sudo('mkdir /dev/hugepages')
        sudo('echo "hugetlbfs       /dev/hugepages  hugetlbfs       defaults        0 0\n" >> /etc/fstab')
        sudo('mount -t hugetlbfs hugetlbfs /dev/hugepages')
        sudo('echo "vm.nr_hugepages = %s" > /etc/sysctl.d/60-hugepages.conf' % pages)
        sudo('sysctl vm.nr_hugepages=%s' % pages)
        sudo("sysctl -p /etc/sysctl.conf")

        # modify libvirt template to enable hugepages
        sudo("sed -i 's#</domain>#\\t<memoryBacking><hugepages/></memoryBacking>\\n</domain>#g' /usr/share/pyshared/nova/virt/libvirt.xml.template")
