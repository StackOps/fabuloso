...
@task
def configure(cluster=False, iface_ex=None):
...
@task
def configure_ovs_plugin_gre(ip_tunnel='127.0.0.1',tunnel_start='1',tunnel_end='1000',mysql_username='quantum',
...
@task
def configure_ovs_plugin_vlan(iface_bridge='eth1',br_postfix='eth1',vlan_start='1',vlan_end='4094',mysql_username='quantum',
                              mysql_password='stackops', mysql_host='127.0.0.1', mysql_port='3306', mysql_schema='quantum'):
    utils.set_option(OVS_PLUGIN_CONF,'sql_connection',utils.sql_connect_string(mysql_host, mysql_password, mysql_port, mysql_schema, mysql_username),section='DATABASE')
    utils.set_option(OVS_PLUGIN_CONF,'reconnect_interval','2',section='DATABASE')
    ...
    utils.set_option(OVS_PLUGIN_CONF,'bridge_mappings','physnet1:br-%s' % iface_bridge,section='OVS')
    utils.set_option(OVS_PLUGIN_CONF,'root_helper','sudo /usr/bin/quantum-rootwrap /etc/quantum/rootwrap.conf',section='AGENT')
    with settings(warn_only=True):
        sudo('ovs-vsctl del-br br-int')
    sudo('ovs-vsctl add-br br-int')
...
@task
def configure_l3_agent(service_user='quantum', service_tenant_name='service', service_pass='stackops', auth_url='http://localhost:35357/v2.0', metadata_ip='127.0.0.1', region='RegionOne'):
...
@task
def configure_external_network(floating_start,floating_end,floating_gw,floating_range, admin_user='admin', admin_tenant_name='admin', admin_pass='stackops', auth_url='http://localhost:5000/v2.0'):
    stdout = sudo('quantum --os-auth-url %s --os-username %s --os-password %s --os-tenant-name %s net-create ext_net --provider:network_type local  --router:external=True' % (auth_url,admin_user,admin_pass, admin_tenant_name))
    external_network_id = get_net_id(stdout)
    sudo('quantum --os-auth-url %s --os-username %s --os-password %s --os-tenant-name %s subnet-create --ip_version 4 --allocation-pool start=%s,end=%s --gateway %s %s %s -- --enable_dhcp=False' % (auth_url,admin_user,admin_pass, admin_tenant_name, floating_start,floating_end,floating_gw,external_network_id,floating_range))
...
@task
def configure_private_network(tenant_id, network_name, private_range, private_gw, external_network_id, admin_user='admin', admin_tenant_name='admin', admin_pass='stackops', auth_url='http://localhost:5000/v2.0'):
    stdout = sudo('quantum --os-auth-url %s --os-username %s --os-password %s --os-tenant-name %s net-create --tenant_id %s %s' % (auth_url,admin_user,admin_pass, admin_tenant_name,tenant_id,network_name))
    private_network_id = get_net_id(stdout)
    stdout = sudo('quantum --os-auth-url %s --os-username %s --os-password %s --os-tenant-name %s subnet-create --tenant_id %s --ip_version 4 %s %s --gateway %s'  % (auth_url,admin_user,admin_pass, admin_tenant_name,tenant_id,private_network_id,private_range,private_gw))
    private_subnet_id = get_net_id(stdout)
    stdout = sudo('quantum --os-auth-url %s --os-username %s --os-password %s --os-tenant-name %s router-create --tenant_id %s provider-router' % (auth_url,admin_user,admin_pass, admin_tenant_name,tenant_id))
    ...
    sudo('quantum --os-auth-url %s --os-username %s --os-password %s --os-tenant-name %s router-gateway-set %s %s'  % (auth_url,admin_user,admin_pass, admin_tenant_name,router_id,external_network_id))

@task
def delete_network(network_id, admin_user='admin', admin_tenant_name='admin', admin_pass='stackops', auth_url='http://localhost:5000/v2.0'):
    sudo('quantum --os-auth-url %s --os-username %s --os-password %s --os-tenant-name %s net-delete %s' % (auth_url,admin_user,admin_pass, admin_tenant_name, network_id))
