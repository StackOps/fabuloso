...
def configure(cluster=False):
    ...
    if cluster:
        ...
        put('./ocf/nova-novnc', '/usr/lib/ocf/resource.d/openstack/nova-novnc', use_sudo=True)
        put('./ocf/nova-api', '/usr/lib/ocf/resource.d/openstack/nova-api', use_sudo=True)
        put('./ocf/nova-cert', '/usr/lib/ocf/resource.d/openstack/nova-cert', use_sudo=True)
        put('./ocf/nova-consoleauth', '/usr/lib/ocf/resource.d/openstack/nova-consoleauth', use_sudo=True)
        put('./ocf/nova-scheduler', '/usr/lib/ocf/resource.d/openstack/nova-scheduler', use_sudo=True)
...
@task
def configure_files(service_user='nova', service_tenant_name='service', service_pass='stackops', auth_host='127.0.0.1',
...
@task
def create_base_configuration(management_ip, mysql_username='nova',
    ...
    set_property('sql_connection',sql_connect_string(mysql_host, mysql_password, mysql_port, mysql_schema, mysql_username))
    ...
    set_property('notification_driver', 'nova.openstack.common.notifier.rabbit_notifier')
